...
fn create_tree(input: Vec<i32>) -> Option<Rc<RefCell<TreeNode>>> {
    let mut root = None;

    for &i in input.iter() {
        root = insert_to_tree(root, i);
    }
...
#[cfg(test)]
mod tests {
    use crate::solutions::s026_invert_binary_tree::compare;
    use crate::solutions::s026_invert_binary_tree::create_tree;
